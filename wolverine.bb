...
Function performGlidingKick(n)
    ...
    If zOnGnd(n)=1 Or downKey(n)=0 Or hitKey(n)=0 Then zBlowSeq(n)=endSeq
...
Function doClawAttack(n)
    ...
    If zBlowSeq(n)=i2+1 Then zBlowSeq(n)=endSeq
...
Case 2    ;Flying Kick
    ...
    If zBlowSeq(n)=1 And downKeyDoubleTap(n)=1 Then 
        zBlowSeq(n)=glidingKickSeq:downKeyDoubleTap(n)=0
    ...
    If zBlowSeq(n) > 1 And zBlowSeq(n) < endSeq Then
    ...
    If zBlowSeq(n) > e And zBlowSeq(n) < clawSeq Then zBlowSeq(n)=0:zBlow(n)=0:zBlowStill(n)=0
Case 4    ;Low kick
    zNoMove(n)=1:zNoJump(n)=1
