...
Case 14	;Super Special
	...
	If zBlowSeq(n) > f And zBlowSeq(n) =< g Then
		...
		Else
			zani(n)=17:zf(n)=6
		EndIf
		If zBlowSeq(n) Mod 5 = 0 And gameSound = 1 Then PlaySound wolverineSlash2Snd 
...
Case 1	;High Claw
	a=6/wolvSpdFctr(n):b=12/wolvSpdFctr(n):c=18/wolvSpdFctr(n):d=24/wolvSpdFctr(n):e=27/wolvSpdFctr(n)
	:f=30/wolvSpdFctr(n):g=34/wolvSpdFctr(n):h=43/wolvSpdFctr(n):i=60/wolvSpdFctr(n)
	zNoMove(n)=1
	zNoJump(n)=1
	...
	If zBlowSeq(n) => f And zBlowSeq(n) < g Then 
		If KeyDown(shotK(n)) = 0 Then zBlowSeq(n) = h
		If zBlowSeq(n) = f And gameSound Then PlaySound wolverineShout2Snd
		zblowPamount(n)=3
		nn=1
...
Case 5	;Uppercut (Tornado claw)
	...
	a=5:b=10:c=16 :d=28 :e=30 
	:f=32 :g=34 :h=36 :i=41 :j=43 
	:k=45 :l=47 :m=52 :nnn=54 :o=56 
	:p=58 :q=63 :r=75 
	If zBlowSeq(n) = 1 Then
	...
	If zBlowSeq(n) > c And zBlowSeq(n) =< d Then
		...
		zBlowDamage(n)=4:zBLowEffect(n)=1:zBlowImpact(n)=30:zBlowStillTime(n)=5:zBlowBlockTime(n)=20
		zBlowSound(n)=wolverineSlash2Snd
	...
	If zBlowSeq(n) > d And zBlowSeq(n) =< r Then
		...
		If zBlowSeq(n) > g And zBlowSeq(n) =< h Then zani(n)=7:zf(n)=8:zantiplat(n)=1:moveY(n,-5):moveX(n,zBlowdir(n),1)
		...
		If zBlowSeq(n) > k And zBlowSeq(n) =< l Then zani(n)=7:zf(n)=4:zantiplat(n)=1:moveY(n,-5.5):moveX(n,zBlowdir(n),1.5)
		...
		If zBlowSeq(n) > o And zBlowSeq(n) =< p Then zani(n)=7:zf(n)=8:zantiplat(n)=1:moveY(n,-6):moveX(n,zBlowdir(n),2)
...
Case 7	;berserker slash (special)
	a=7/wolvSpdFctr(n):b=14/wolvSpdFctr(n):c=21/wolvSpdFctr(n):d=28/wolvSpdFctr(n):e=35/wolvSpdFctr(n)
	:f=42/wolvSpdFctr(n):g=49/wolvSpdFctr(n):h=56/wolvSpdFctr(n)
	zNoMove(n)=1
	zNoJump(n)=1
	zjump(n)=0
...
Case 16 ;Counter Key (Berserker Rage)
	...
	If zBlowSeq(n) >= nnn And zBlowSeq(n) <= o Then
		...
		zHitmode(n)=2:zBlowHold(n)=0:zBlowSound(n)=wolverineSlash2Snd
...
Case 17 ;Extra special key (Drill Claw)
	...
	If zBlowSeq(n) => e And zBlowSeq(n) < f Then zani(n)=4:zf(n)=1:zNoGrav(n)=0:ztopSpeed(n)=5:zNomove(n)=0
