Function doSubZeroTaunt(n)
    ...
    If zTauntSeed(n)=1 Then
        ...
        If zBlowSeq(n) > d And zBlowSeq(n) <= e Then
            ...
            If zf(n)>17 Then zf(n)=17
        End If
    Else    ;Buy Sub Zero doll taunt
        superbarGain=5
        If zBlowSeq(n)>a2 Then extraObj(n,zx(n),5,zy(n),-80,zblowdir(n),125)
        If zBlowSeq(n)=a2 And gameSound Then PlaySound mkFriendShipAgainSnd
        If zBlowSeq(n)>0 And zBlowSeq(n)<=a2 Then zf(n)=6
...
Function performFatalitySuper(n)
    ...
    If zBlowSeq(n) => c And zBlowSeq(n) < c+5
        ...
        zBlowDamage(n)=14:zBLowEffect(n)=1:zEnemyImmuneTime(n)=99:zBlowStillTime(n)=0:zBlowBlockTime(n)=35
...
;--------- Sound ------------
    If zBlowSeq(n) = seq2-2 And gameSound Then PlaySound mkPunchSnd
...
Case 2    ;Flying kick
    ...
    If zBlowSeq(n) >= upFlyKickSeq Then doSubZeroUpFlyKick(n)
    ...
    If zBlowSeq(n) > b And zBlowSeq(n) < c Then
        ...
        zBlowSound(n)=mkHitSnd
    EndIf
    If zBlowSeq(n)=endSeq And zBlowSeq(n) < upFlyKickSeq Then zBlowSeq(n)=0:zBlow(n)=0:zBlowStill(n)=0
...
Case 4    ;Low kick
    ...
    If zBlowSeq(n) > a And zBlowSeq(n) =< b Then 
        ...
        zBlowDamage(n)=10:zBLowEffect(n)=1:zEnemyImmuneTime(n)=99:zBlowStillTime(n)=12:zBlowBlockTime(n)=30
        zBlowSound(n)=mkHitSnd
        zani(n)=9:zf(n)=2
    EndIf
