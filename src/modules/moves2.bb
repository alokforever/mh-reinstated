Function doTurtleSlidePunchSeq(n)
    seqStart=100:seq1=seqStart+5:seq2=seq1+12:seq3=seq2+21:seq4=seq3+3
    endSeq=21
    
;---------- Animations -------------
    If zBlowSeq(n)>=seqStart And zBlowSeq(n)<seq1 Then zF(n)=1
    If zBlowSeq(n)>=seq1 And zBlowSeq(n)<seq3 Then zF(n)=6
    If zBlowSeq(n)>=seq3 And zBlowSeq(n)<seq4 Then zF(n)=1

;------------- Sound ---------------
    If zBlowSeq(n)=seq1-1 And gameSound Then
        randSnd=Rand(10)
        If randSnd <= 4 Then PlaySound ktGrunt2Snd
        PlaySound turtleSlideSnd
    End If

;----------- Movement --------------
    If zBlowSeq(n)>=seq1 And zBlowSeq(n)<seq2 Then movex2(n,zBlowdir(n),2.0)
    If zBlowSeq(n)>=seq2 And zBlowSeq(n)<seq3-6 Then movex2(n,zBlowdir(n),0.5)

;----------- Hitbox --------------
    If zf(n)=6 Then
        zblowPamount(n)=3:nn=1
        xblow(n,nn)=6.40063:yblow(n,nn)=24.0:wblow(n,nn)=18:hblow(n,nn)=6:nn=nn+1
        xblow(n,nn)=7.40063:yblow(n,nn)=20.0:wblow(n,nn)=11:hblow(n,nn)=19:nn=nn+1
        xblow(n,nn)=18.4006:yblow(n,nn)=17.0:wblow(n,nn)=15:hblow(n,nn)=15:nn=nn+1
...
Case 10     ;Belly jump hit
    ...
    If zani(n)=6 And zf(n)=2 And zblowseq(n) > 4 Then
        ...
        xblow(n,nn)=-25.3222:yblow(n,nn)=18.9992:wblow(n,nn)=14:hblow(n,nn)=18:nn=nn+1
        xblow(n,nn)=-12.3222:yblow(n,nn)=19.9992:wblow(n,nn)=18:hblow(n,nn)=10:nn=nn+1
        xblow(n,nn)=4.67776:yblow(n,nn)=20.9992:wblow(n,nn)=17:hblow(n,nn)=11:nn=nn+1
        xblow(n,nn)=-11.3222:yblow(n,nn)=11.9992:wblow(n,nn)=13:hblow(n,nn)=11:nn=nn+1
        xblow(n,nn)=2.67776:yblow(n,nn)=11.9992:wblow(n,nn)=19:hblow(n,nn)=13:nn=nn+1
        xblow(n,nn)=0: yblow(n,nn)=6:wblow(n,nn)=11:hblow(n,nn)=1
        zHitMode(n)=0:zBlowHold(n)=8:zBlowBack(n)=1
...
Case 1    ;Belly hit
    ...
    If zani(n)=6 And zf(n)=1 And zblowseq(n) > 3 And zBlowSeq(n) < d Then
        ...
        xblow(n,nn)=-1.88121:yblow(n,nn)=44.1999:wblow(n,nn)=25:hblow(n,nn)=14:nn=nn+1
        xblow(n,nn)=-0.88121:yblow(n,nn)=29.1999:wblow(n,nn)=24:hblow(n,nn)=6:nn=nn+1
        xblow(n,nn)=0.11879:yblow(n,nn)=23.1999:wblow(n,nn)=19:hblow(n,nn)=6:nn=nn+1
        xblow(n,nn)=1.11879:yblow(n,nn)=18.1999:wblow(n,nn)=12:hblow(n,nn)=7:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=8:zBlowBack(n)=1
...
Case 1    ;High Punch
    ...
    If zBlowSeq(n) => b And zBlowSeq(n) < c Then
        zblowPamount(n)=3:nn=1
        xblow(n,nn)=13.5015:yblow(n,nn)=71.0:wblow(n,nn)=11:hblow(n,nn)=22:nn=nn+1
        xblow(n,nn)=24.5015:yblow(n,nn)=73.0:wblow(n,nn)=9:hblow(n,nn)=17:nn=nn+1
        xblow(n,nn)=33.5015:yblow(n,nn)=71.0:wblow(n,nn)=13:hblow(n,nn)=13:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=8
...
Case 2    ;Flying Kick
    ...
    If zBlowSeq(n) > a And zBlowSeq(n) =< b Then
        zblowPamount(n)=2:nn=1
        xblow(n,nn)=13.3593:yblow(n,nn)=24.8:wblow(n,nn)=21:hblow(n,nn)=16:nn=nn+1
        xblow(n,nn)=34.3593:yblow(n,nn)=29.8:wblow(n,nn)=21:hblow(n,nn)=20:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=8
...
Case 4    ;Low kick
    ...
    If zBlowSeq(n) => b And zBlowSeq(n) =< c Then
        zblowPamount(n)=3:nn=1
        xblow(n,nn)=14.3595:yblow(n,nn)=23.0:wblow(n,nn)=10:hblow(n,nn)=21:nn=nn+1
        xblow(n,nn)=24.3595:yblow(n,nn)=19.0:wblow(n,nn)=11:hblow(n,nn)=20:nn=nn+1
        xblow(n,nn)=36.3595:yblow(n,nn)=13.0:wblow(n,nn)=16:hblow(n,nn)=17:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=8
...
Case 1    ;Kick Attack
    zNoMove(n)=1:zNoJump(n)=1
    seq1=2:seq2=4:seq3=7:seq4=13:seq5=17:seq6=21
    
;----------- Sounds -----------
    If zBlowSeq(n)=seq3-1 And gameSound Then
        PlaySound ktKickSnd
        PlaySound ktGrunt1Snd
    End If
    
;---------- Animation -------------
    If zBlowSeq(n) >= 1 And zBlowSeq(n) <= seq1 Then zani(n)=6:zf(n)=1
...
;---------- Hit box --------------
    If zF(n)=4 Then
        zblowPamount(n)=2:nn=1
        xblow(n,nn)=14.2:yblow(n,nn)=35.0:wblow(n,nn)=7:hblow(n,nn)=15:nn=nn+1
        xblow(n,nn)=20.2:yblow(n,nn)=40.0:wblow(n,nn)=10:hblow(n,nn)=18:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=8
        zBlowDamage(n)=10:zBLowEffect(n)=1:zEnemyImmuneTime(n)=99:zBlowStillTime(n)=12:zBlowBlockTime(n)=25
        zBlowSound(n)=mvcHit2Snd
    End If
    
    If zBlowSeq(n) > seq6 Then zBlowSeq(n)=0:zBlow(n)=0
    
Case 2    ;Flying Kick
    zNoJump(n)=0:ZJUMPING(N)=1
    seq1=4:seq2=12:seq3=16

;----------- Sounds --------------
    If zBlowSeq(n)=seq2-1 And gameSound Then
        randSeed=Rand(2)
        PlaySound ktKickSnd
        If randSeed=1 Then PlaySound ktGrunt1Snd
        If randSeed=2 Then PlaySound ktGrunt2Snd
    End If
...
;---------- Hit box --------------
    If zBlowSeq(n) > seq2 And zBlowSeq(n) <= seq3 Then
        ...
        xblow(n,nn)=7.79984:yblow(n,nn)=34.6:wblow(n,nn)=18:hblow(n,nn)=16:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=8
        zBlowDamage(n)=12:zBLowEffect(n)=1:zEnemyImmuneTime(n)=99:zBlowStillTime(n)=12:zBlowBlockTime(n)=25
        zBlowSound(n)=mvcHit2Snd
    End If
    
    If zBlowSeq(n) > seq3 Then zBlowSeq(n)=0:zBlow(n)=0
    
Case 4    ;Low kick
    zNoMove(n)=1:zNoJump(n)=1
    zheight(n)=zduckheight(n)
    seq1=3:seq2=7:seq3=13:seq4=18:seq5=21
    slidePunchSeq=100:zani(n)=9
    
    If zBlowSeq(n)=1 And (leftKey(n)=1 Or rightKey(n)=1) Then
        zBlowSeq(n)=slidePunchSeq
    End If
    
    If zBlowSeq(n) >= slidePunchSeq Then doTurtleSlidePunchSeq(n)
...
;---------- Animation ------------
    If zBlowSeq(n) >= 1 And zBlowSeq(n) <= seq1 Then zf(n)=1
...
;---------- Hit box --------------
    If zf(n)=3 Then
        zblowPamount(n)=1:nn=1
        xblow(n,nn)=-22.4781:yblow(n,nn)=25.0:wblow(n,nn)=17:hblow(n,nn)=17:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=8
        zBlowDamage(n)=13:zBLowEffect(n)=1:zEnemyImmuneTime(n)=99:zBlowStillTime(n)=10:zBlowBlockTime(n)=30
        zBlowSound(n)=mvcHit2Snd
    EndIf

    If zBlowSeq(n) > seq5 And zBlowSeq(n) < slidePunchSeq Then zBlowSeq(n)=0:zBlow(n)=0

Case 5    ;Up + Special (Flash kick)
    zJump(n)=0:zNoJump(n)=1:zNograv(n)=1:zJumping(n)=0

    seq1=2:seq2=4:seq3=6:seq4=9:seq5=seq4+6:seq6=seq5+4:seq7=seq6+5
    seq8=seq7+6:seq8=seq7+13
...
;---------- Animation ------------
    ...
    If zBlowSeq(n) > seq7 And zBlowSeq(n) Mod 3 = 0 Then
        zani(n)=5
        If zF(n)=2 Then
            zF(n)=3
        Else If zF(n)=3 Then
    ...
    If zBlowSeq(n)=seq5 Then extraObj(n,zx(n),20,zy(n),0,zBlowDir(n),167)
    
;---------- Movement -------------
    If zBlowStill(n)=0 Then
        If zBlowSeq(n) <= seq5 Then moveX(n,zBlowdir(n),3):moveY(n,-3.3)
        If zBlowSeq(n) > seq5 And zBlowSeq(n) <= seq7 Then moveX(n,zBlowdir(n),1):moveY(n,-1.1)
        If zBlowSeq(n) > seq7 And zBlowSeq(n) <= seq8 Then moveX(n,zBlowdir(n),1)
    End If
    
    If zBlowSeq(n) > seq7 Then zNoGrav(n)=0
    
;---------- Hit box --------------
    If zBlowSeq(n) >= seq5 And zBlowSeq(n) <= seq5+1 Then
        zblowPamount(n)=11:nn=1
        xblow(n,nn)=8.36133:yblow(n,nn)=11.0396:wblow(n,nn)=13:hblow(n,nn)=7:nn=nn+1
        xblow(n,nn)=17.3613:yblow(n,nn)=21.0396:wblow(n,nn)=14:hblow(n,nn)=11:nn=nn+1
        xblow(n,nn)=31.3613:yblow(n,nn)=28.0396:wblow(n,nn)=8:hblow(n,nn)=13:nn=nn+1
        xblow(n,nn)=19.3613:yblow(n,nn)=38.0396:wblow(n,nn)=14:hblow(n,nn)=10:nn=nn+1
        xblow(n,nn)=33.3613:yblow(n,nn)=38.0396:wblow(n,nn)=15:hblow(n,nn)=11:nn=nn+1
        xblow(n,nn)=11.3613:yblow(n,nn)=47.0396:wblow(n,nn)=19:hblow(n,nn)=8:nn=nn+1
        xblow(n,nn)=29.3613:yblow(n,nn)=51.0396:wblow(n,nn)=14:hblow(n,nn)=7:nn=nn+1
        xblow(n,nn)=13.3613:yblow(n,nn)=64.0396:wblow(n,nn)=17:hblow(n,nn)=10:nn=nn+1
        xblow(n,nn)=31.3613:yblow(n,nn)=65.0396:wblow(n,nn)=12:hblow(n,nn)=6:nn=nn+1
        xblow(n,nn)=7.36133:yblow(n,nn)=80.0396:wblow(n,nn)=21:hblow(n,nn)=13:nn=nn+1
        xblow(n,nn)=-5.63867:yblow(n,nn)=65.0396:wblow(n,nn)=15:hblow(n,nn)=16:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=8
...
;----------- Sounds --------------
    If zBlowSeq(n)=seq3-1 And gameSound Then PlaySound ktGrunt3Snd
    
    If zBlowSeq(n) < seq8 Then
        zNoMove(n)=1
    Else
        zNoMove(n)=0:ztopSpeed(n)=1
    End If
    
    If zOnGnd(n)=1 And zBlowSeq(n) > seq8 Then zBlowSeq(n)=0:zBlow(n)=0:zblowstill(n)=0

Case 6    ;throwing iten
    a=2:b=3:c=6:d=8
    
    If zongnd(n)=1 Then zNoMove(n)=1:zNoJump(n)=1
    If zBlowSeq(n) => 1 And zBlowSeq(n) =< a Then zani(n)=6:zf(n)=7
    If zBlowSeq(n) => a And zBlowSeq(n) < b Then zani(n)=6:zf(n)=8
    If zBlowSeq(n)= b Then
        throwObj(n,zx(n),zy(n)-20,zFace(n))
        If gameSound Then PlaySound throwsnd
    EndIf
    If zBlowSeq(n) => b And zBlowSeq(n) < c Then zani(n)=6:zf(n)=9
    If zBlowSeq(n) => c And zBlowSeq(n) < d Then zani(n)=6:zf(n)=10
...
Case 11    ;club
    ...
    If zBlowSeq(n) => 1 And zBlowSeq(n) =< a Then zani(n)=6:zf(n)=5 :eAni(n)=1:ef(n)=2:xed(n)=-28:yed(n)=40
    If zBlowSeq(n) => a And zBlowSeq(n) =< b Then zani(n)=6:zf(n)=7 :eAni(n)=1:ef(n)=2:xed(n)=-46:yed(n)=45
    If zBlowSeq(n)= a Then If gameSound Then PlaySound voosnd
    If zBlowSeq(n) => b And zBlowSeq(n) =< c Then
        zblowPamount(n)=7:nn=1
        xblow(n,nn)=20.3602:yblow(n,nn)=33.0:wblow(n,nn)=16:hblow(n,nn)=14:nn=nn+1
        xblow(n,nn)=37.3602:yblow(n,nn)=34.0:wblow(n,nn)=17:hblow(n,nn)=16:nn=nn+1
        xblow(n,nn)=55.3602:yblow(n,nn)=50.0:wblow(n,nn)=19:hblow(n,nn)=15:nn=nn+1
        xblow(n,nn)=75.3602:yblow(n,nn)=41.0:wblow(n,nn)=17:hblow(n,nn)=13:nn=nn+1
        xblow(n,nn)=93.3602:yblow(n,nn)=39.0:wblow(n,nn)=18:hblow(n,nn)=24:nn=nn+1
        xblow(n,nn)=55.3602:yblow(n,nn)=33.0:wblow(n,nn)=20:hblow(n,nn)=25:nn=nn+1
        xblow(n,nn)=76.3602:yblow(n,nn)=28.0:wblow(n,nn)=19:hblow(n,nn)=22:nn=nn+1
        ...
        zani(n)=6:zf(n)=9
        eAni(n)=1:ef(n)=3:xED(n)=65:yed(n)=4
    EndIf
    If zBlowSeq(n) => c And zBlowSeq(n) =< d Then zani(n)=6:zf(n)=9 :eAni(n)=1:ef(n)=4:xed(n)=65:yed(n)=4
    If zBlowSeq(n) => d And zBlowSeq(n) =< e Then zani(n)=6:zf(n)=9 :eAni(n)=1:ef(n)=4:xed(n)=65:yed(n)=4
    If zBlowSeq(n) > e Then zBlowSeq(n)=0:zBlow(n)=0

Case 9    ;Down + Special (Hold punch)
    zNoMove(n)=1:zNoJump(n)=1:zNoGrav(n)=1
    seq1=30:seq2=seq1+4:seq3=seq2+30:seq4=seq3+6:seq5=seq4+3
...
;---------- Animation ------------
    ...
    If zBlowSeq(n) > seq5 Then zani(n)=4:zf(n)=1
    
    If zface(n)=2 Then x=zx(n)+35
    If zface(n)=4 Then x=zx(n)-35
    If zBlowSeq(n)=seq2 Then makeChunk(n,x,zy(n)+65,shotDir(n),166)

;------------ Sounds -------------
    If zBlowSeq(n) = seq2-1 And gameSound Then
        PlaySound ktGrunt4Snd
        PlaySound ktKartExplodeSnd
    End If
    
;---------- Hit box --------------
    If zBlowSeq(n) >= seq2 And zBlowSeq(n) <= seq3-15 Then
        zblowPamount(n)=11:nn=1
        xblow(n,nn)=-7.23907:yblow(n,nn)=96.0:wblow(n,nn)=26:hblow(n,nn)=22:nn=nn+1
        xblow(n,nn)=18.7609:yblow(n,nn)=91.0:wblow(n,nn)=33:hblow(n,nn)=22:nn=nn+1
        xblow(n,nn)=-2.23907:yblow(n,nn)=74.0:wblow(n,nn)=26:hblow(n,nn)=23:nn=nn+1
        xblow(n,nn)=23.7609:yblow(n,nn)=68.0:wblow(n,nn)=31:hblow(n,nn)=19:nn=nn+1
        xblow(n,nn)=-18.2391:yblow(n,nn)=53.0:wblow(n,nn)=29:hblow(n,nn)=30:nn=nn+1
        xblow(n,nn)=-15.2391:yblow(n,nn)=22.0:wblow(n,nn)=30:hblow(n,nn)=23:nn=nn+1
        xblow(n,nn)=-0.239075:yblow(n,nn)=-1.0:wblow(n,nn)=30:hblow(n,nn)=13:nn=nn+1
        xblow(n,nn)=13.7609:yblow(n,nn)=50.0:wblow(n,nn)=27:hblow(n,nn)=23:nn=nn+1
        xblow(n,nn)=41.7609:yblow(n,nn)=46.0:wblow(n,nn)=29:hblow(n,nn)=20:nn=nn+1
        xblow(n,nn)=20.7609:yblow(n,nn)=23.0:wblow(n,nn)=19:hblow(n,nn)=18:nn=nn+1
        xblow(n,nn)=40.7609:yblow(n,nn)=22.0:wblow(n,nn)=24:hblow(n,nn)=28:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=8
        zBlowDamage(n)=13:zBLowEffect(n)=1:zEnemyImmuneTime(n)=99:zBlowStillTime(n)=1:zBlowBlockTime(n)=30
        zBlowSound(n)=mvcHit2Snd
    EndIf
    
;---------- Movement -------------
    If zBlowStill(n)=0 Then
        If zF(n)=2 Then moveX(n,zBlowDir(n),14)
        If zBlowSeq(n) > seq2 And zBlowSeq(n) <= seq3-20 Then moveX(n,zBlowDir(n),0.5)
    End If
    
    If zBlowSeq(n) > seq3 Then zNoGrav(n)=0:zNoMove(n)=0:ztopSpeed(n)=1
    If zBlowSeq(n) > seq5 And zOnGnd(n)=1 Then zBlowSeq(n)=0:zBlow(n)=0:zblowstill(n)=0
...
;=========== Sounds ===========
        If gameSound Then
            PlaySound ktOneMoreTimeSnd:PlaySound mvcSuper1Snd
        End If
    End If
    If zOnGnd(n)=1 Then
        If zBlowSeq(n)=kartMoveSeq+1 Or zBlowSeq(n) Mod 50=0 Then
            If gameSound Then PlaySound ktKartScreechSnd
        End If
    End If
    ...
    If zBlowSeq(n)=2 Then
        ...
        zSuperMove(n)=1
        checkChunk(n)=makechunk(n,zx(n),zy(n)-140,zFace(n),165)
    End If
    
    If zBlowSeq(n)>3 And zBlowSeq(n) < kartMoveSeq And chunk(checkChunk(n))=0 And zOnGnd(n)=1 Then
        zBlowSeq(n)=kartMoveSeq:zani(n)=17
    End If
    
;=========== Animation ===========
    If zBlowSeq(n) > kartMoveSeq Then
        If chunkSeq(n) Mod 3 = 0 Then 
            ...
            End If
        End If
        local speed#=(zBlowSeq(n)-kartMoveSeq)/4
        if speed > 11 Then speed=11
        moveX(n,zBlowdir(n),speed#)
        If zOnGnd(n)=0 Then zBlowSeq(n)=zBlowSeq(n)-1:zy(n)=zy(n)+2
        
        If speed > 1 Then
            checkZvsWall(n,1):zTempStone(n)=1
            zblowPamount(n)=7:nn=1:zBlowBack(n)=1
            xblow(n,nn)=-5.40002:yblow(n,nn)=66.0:wblow(n,nn)=19:hblow(n,nn)=20:nn=nn+1
            xblow(n,nn)=-5.40002:yblow(n,nn)=47.0:wblow(n,nn)=21:hblow(n,nn)=19:nn=nn+1
            xblow(n,nn)=-4.40002:yblow(n,nn)=30.0:wblow(n,nn)=20:hblow(n,nn)=13:nn=nn+1
            xblow(n,nn)=-5.40002:yblow(n,nn)=18.0:wblow(n,nn)=19:hblow(n,nn)=15:nn=nn+1
            xblow(n,nn)=13.6:yblow(n,nn)=29.0:wblow(n,nn)=10:hblow(n,nn)=13:nn=nn+1
            xblow(n,nn)=22.6:yblow(n,nn)=20.0:wblow(n,nn)=14:hblow(n,nn)=14:nn=nn+1
            xblow(n,nn)=13.6:yblow(n,nn)=16.0:wblow(n,nn)=12:hblow(n,nn)=16:nn=nn+1
            zHitMode(n)=0:zBlowHold(n)=8
            zBlowDamage(n)=50:zBLowEffect(n)=1:zEnemyImmuneTime(n)=99:zBlowStillTime(n)=8:zBlowBlockTime(n)=30
            zBlowSound(n)=mvcHit2Snd
        End If

        If zBlowStill(n)=1 Or isHitWall(n)=1 Then
            makeExp(n,zx(n),zy(n),3)
            makeChunk(n,zx(n),zy(n)+30,shotDir(n),166)
...
Case 16 ;Counter Key (Taunt)
    ...
    If zBlowSeq(n)=1 And gameSound Then PlaySound coinsnd
    If zBlowSeq(n)>=1 And zBlowSeq(n)<=endSeq Then zF(n)=1
...
Case 17 ; Extra special key (Flight)
    ...
    endSeq=728:zNoMove(n)=0
    
    If zBlowSeq(n)=1 And zStaminaBar#(n) < 30 Then isFlashLowStamina(n)=1:zBlowSeq(n)=endSeq
    If zBlowSeq(n) > 1 And zStaminaBar#(n) <= 0 Then zBlowSeq(n)=endSeq Else depleteStaminaBar(n, 0.7)
    
    If zBlowSeq(n)=1 Then
        maxFlightYLimit(n)=zy(n)-200
    End If
    
    If (shotkey(n) Or grabKey(n)) And zGotObj(n)>0 Then
        If zFace(n)=2 Then throwObj(n,zx(n)+10,zy(n)-30,zFace(n))
        If zFace(n)=4 Then throwObj(n,zx(n)-10,zy(n)-30,zFace(n))
        If gameSound Then PlaySound throwsnd
    End If
    
    If rightKey(n)=1 Then zFace(n)=2:zBlowDir(n)=2
    If leftKey(n)=1 Then zFace(n)=4:zBlowDir(n)=4
    
;--------------------- Animation --------------------
    If zBlowSeq(n) > 0 And zBlowSeq(n) <= 3 Then zani(n)=4:zf(n)=1
    If zBlowSeq(n) > 3 And zBlowSeq(n) <= 6 Then zani(n)=18:zf(n)=1
    If zBlowSeq(n) > 6 And zBlowSeq(n) <= endSeq And zBlowSeq(n) Mod 6 = 0 Then
        If zF(n)=1 Then
            zF(n)=2
        Else If zF(n)=2 Then
            zF(n)=3
...
;--------------------- Movement --------------------
    If zBlowSeq(n) > 0 And zBlowSeq(n) <= 6 Then
        zNoMove(n)=1:moveY(n,-3)
    Else
        If zBlowSeq(n) <= endSeq Then
            If upKey(n)=1 And zy(n) > maxFlightYLimit(n) And zHitHead(n)=0 Then moveY(n,-3)
            If downKey(n)=1 Then moveY(n,3)
        End If
    End If

    If zBlowSeq(n) > 6 And extraSpecialkey(n)=1 Then zBlowSeq(n)=endSeq
    If zBlowSeq(n) >= endSeq And zOnGnd(n)=0 Then 
        zNoGrav(n)=0:ztopSpeed(n)=1:zNomove(n)=0
        zani(n)=4:zF(n)=1
    End If
    If zBlowSeq(n) >= endSeq And zOnGnd(n)=1 Then zBlowSeq(n)=0:zBlow(n)=0

Default     ;spinning shell
    seq1=3:seq2=6:seq3=8:seq4=10:seq5=12:seq6=14:seq7=16
...
;--------------------- Animation --------------------
    ...
    If zBlowSeq(n) > seq8 And zBlowSeq(n) <= seq9 Then zF(n)=2:zNoMove(n)=1
    If zBlowSeq(n) > seq9 And zBlowSeq(n) <= seq10 Then zF(n)=1:zNoMove(n)=1
    
;--------------------- Sounds ----------------------
    If zBlowSeq(n)=2 And gameSound Then PlaySound ktSpinSnd
    
;--------------------- Hitbox --------------------
    If zf(n)>=3 And zf(n)<=7 Then
        zblowPamount(n)=3:nn=1
        xblow(n,nn)=-14.3871:yblow(n,nn)=22.0:wblow(n,nn)=14:hblow(n,nn)=24:nn=nn+1
        xblow(n,nn)=-0.387085:yblow(n,nn)=22.0:wblow(n,nn)=9:hblow(n,nn)=22:nn=nn+1
        xblow(n,nn)=9.61292:yblow(n,nn)=14.0:wblow(n,nn)=7:hblow(n,nn)=13:nn=nn+1
        zHitMode(n)=0:zBlowHold(n)=12
        zBlowDamage(n)=8:zBLowEffect(n)=1:zEnemyImmuneTime(n)=30:zBlowStillTime(n)=12:zBlowBlockTime(n)=30
