...
Case 15 ;sonya throw
	...
	If zblowseq(n) = f Then
		If gameSound Then PlaySound sonyaSnd
	EndIf
...
Case 14	;Super Special
	...
	If zBlowSeq(n) = 1 And gameSound=1 Then PlaySound sonyaBreathSnd
	If zBlowSeq(n) => 1 And zBlowSeq(n) =< a Then zani(n)=10:zf(n)=4
	If zBlowSeq(n) > a And zBlowSeq(n) < b Then zani(n)=10:zf(n)=5
	If zBlowSeq(n) =b-2 Then
		checkDist(n,zx(n),zy(n)-20,zFace(n))
		zSuperMove(n)=1:zSuperMoveSeq(n)=0:
	EndIf
	If zBlowSeq(n) > b And zBlowSeq(n) < c Then zani(n)=10:zf(n)=6
	If zBlowSeq(n)= c Then 
		dir=zface(n):y=zy(n)-zheight(n)+20
		If zface(n)=2 Then x=zx(n)+10
		If zface(n)=4 Then x=zx(n)-10
		makeshot(n,65,x,y,dir)
	EndIf
	If zBlowSeq(n) = c And gameSound = 1 Then PlaySound sonyaFlameSnd
	If zBlowSeq(n) => c And zBlowSeq(n) =< d Then zani(n)=10:zf(n)=7
...
Case 1	;High Punch
	...
	If zBlowSeq(n) > a And zBlowSeq(n) =< b Then
		zblowPamount(n)=2
		xblow(n,nn)=0: yblow(n,nn)=41:wblow(n,nn)=27:hblow(n,nn)=1:nn=nn+1
		xblow(n,nn)=0: yblow(n,nn)=35:wblow(n,nn)=27:hblow(n,nn)=1
		zHitMode(n)=0:zBlowHold(n)=8
		zBlowDamage(n)=15:zBLowEffect(n)=1:zBlowImpact(n)=20:zBlowStillTime(n)=10:zBlowBlockTime(n)=30
	...
	If zBlowSeq(n) > c And zBlowSeq(n) =< d Then
		zblowPamount(n)=2
		xblow(n,nn)=0: yblow(n,nn)=39:wblow(n,nn)=27:hblow(n,nn)=1:nn=nn+1
		xblow(n,nn)=0: yblow(n,nn)=35:wblow(n,nn)=27:hblow(n,nn)=1
		zHitMode(n)=0:zBlowHold(n)=8
		zBlowDamage(n)=12:zBLowEffect(n)=1:zBlowImpact(n)=99:zBlowStillTime(n)=10
		zBlowSound(n)=Highpunchsnd
		zani(n)=6:zf(n)=7
	EndIf
		If zBlowSeq(n) > d And zBlowSeq(n) =< d Then zani(n)=6:zf(n)=3
	If zBlowSeq(n) > d Then zBlowSeq(n)=0:zBlow(n)=0
...
Case 9	;spinning kick
	...
	If zBlowSeq(n) =a And gameSound Then PlaySound sonyaspinsnd
	...
	If zblowseq(n) => f Then zblowseq(n)=b:zblowseq2(n)=zblowseq2(n)+1
		...
		zBlowDamage(n)=10:zBLowEffect(n)=1:zBlowImpact(n)=20:zBlowStillTime(n)=12:zBlowBlockTime(n)=36
		zBlowSound(n)=kicksnd
		EndIf
		
		If zf(n)=5 Or zf(n)=6 Then
		zblowpamount(n)=2:zblowback(n)=1
		xblow(n,1)=-18: yblow(n,1)=30:wblow(n,1)=20:hblow(n,1)=1
		xblow(n,2)=-18: yblow(n,2)=22:wblow(n,2)=20:hblow(n,2)=1
		zHitmode(n)=5:zBlowHold(n)=0
		zHitSpeed#(n)=5:zHitUpSpeed#(n)=1.5:zHitTime(n)=50
		If zBlowStill(n)=0 Then moveX(n,zBlowdir(n),10)
		zBlowDamage(n)=12:zBLowEffect(n)=1:zBlowImpact(n)=99:zBlowStillTime(n)=6
		zBlowSound(n)=kicksnd
		EndIf
...
Case 10	;High Kick
	...
	If zBlowSeq(n) > b And zBlowSeq(n) =< c Then
		zblowPamount(n)=4
		xblow(n,1)=5: yblow(n,1)=55:wblow(n,1)=1:hblow(n,1)=20
		xblow(n,2)=10: yblow(n,2)=55:wblow(n,2)=1:hblow(n,2)=20
		xblow(n,3)=10: yblow(n,3)=50:wblow(n,3)=1:hblow(n,3)=20
		xblow(n,4)=20: yblow(n,3)=45:wblow(n,4)=1:hblow(n,4)=20
		zHitMode(n)=0:zBlowHold(n)=8
		zBlowDamage(n)=16:zBLowEffect(n)=1:zBlowImpact(n)=99:zBlowStillTime(n)=10
		zBlowSound(n)=kicksnd
		zani(n)=14:zf(n)=3
	EndIf
